# Aprobación interna de lanzamiento

## Congelamiento

- Fecha y hora: ______________________________
- Commit congelado: __________________________
- Versión Android / versionCode: ______________
- SHA-256 del AAB: ____________________________
- Pista interna probada: ______________________

## Evidencias

- [ ] Backend saludable y base conectada.
- [ ] Nueve documentos legales v2.0 activos.
- [ ] Viaje completo pasajero/conductor probado.
- [ ] Pago Mercado Pago probado.
- [ ] Cierre efectivo y beneficio probado.
- [ ] Eliminación de cuenta probada.
- [ ] Rutas futuras responden 404/no disponibles.
- [ ] AAB firmado probado en pista interna.
- [ ] Data Safety revisado contra el AAB.
- [ ] Notas y cuentas de revisión cargadas.

## Firmas

| Área | Nombre | Fecha | Estado / firma |
|---|---|---|---|
| Desarrollo | | | |
| Soporte | | | |
| Jurídica / Privacidad | | | |
| Gerencia | | | |

## Cierre iOS

Proyecto, Sign in with Apple, geolocalización, App Privacy y build iOS deben adjuntarse como evidencia del responsable de Desarrollo antes del envío a App Store.
