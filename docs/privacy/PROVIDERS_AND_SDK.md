...
| Apple Sign in with Apple | Activo en iOS al configurar Apple Developer | autenticación opcional y vinculación segura | identificador estable `sub`, correo/nombre autorizados, estado relay y tokens OAuth cifrados | App iOS y backend auth | Verificar token con JWKS; no fusionar por correo; claves `.p8` solo en backend |
...
| Capacitor Geolocation + plugin nativo RapaGoBackgroundLocation | Activo Android/iOS | ubicación durante solicitud y viaje; segundo plano solo durante viaje activo | coordenadas, precisión, rumbo, velocidad y estado técnico | Frontend nativo y API de tracking | Permiso contextual, indicador de iOS, detener al terminar/cancelar y no conservar ubicación previa innecesaria |
