import type { LoginRequest, RegisterRequest, AuthServiceResult, AuthUser } from "./auth.types.js";
...
export class AuthService {
  async register(payload: RegisterRequest): Promise<AuthServiceResult> {
    ...
    try {
      ...
      if (err instanceof AppError && err.code === "AUTH_EMAIL_TAKEN") {
        ...
        return { ok: false, code: "AUTH_EMAIL_TAKEN", message: "Email address is already registered." };
      }
      auditService.recordSafe({
        eventType:  "auth.register.failure",
        entityType: "user",
  ...
  async login(payload: LoginRequest): Promise<AuthServiceResult> {
    ...
    if (!user) {
      return { ok: false, code: "AUTH_INVALID_CREDENTIALS", message: "Invalid email or password." };
    }
    ...
    if (credentials.lockedUntil && credentials.lockedUntil > new Date()) {
      ...
      return { ok: false, code: "AUTH_ACCOUNT_LOCKED", message: "Account is temporarily locked. Please try again later." };
    }

    const valid = await passwordService.verifyPassword(credentials.passwordHash, payload.password);
    if (!valid) {
      ...
      auditService.recordSafe({
        ...
        entityId:    user.id,
        actorUserId: user.id,
        metadata:    { reason: "invalid_password", attempts: updated.failedLoginAttempts },
      });
      return { ok: false, code: "AUTH_INVALID_CREDENTIALS", message: "Invalid email or password." };
    }
  ...
  async getMe(accessToken: string): Promise<AuthServiceResult> {
    ...
    try {
      ...
      return { ok: false, code: "UNAUTHORIZED", message: "Invalid access token." };
    }

    const hash = tokenService.hashToken(accessToken);
    const sessionValid = await sessionService.isSessionValid(hash);
    if (!sessionValid) {
      return { ok: false, code: "AUTH_SESSION_REVOKED", message: "Session has been revoked." };
    }
