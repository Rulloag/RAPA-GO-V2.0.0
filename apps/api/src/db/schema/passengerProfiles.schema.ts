...
export const passengerProfiles = pgTable("passenger_profiles", {
  ...
  phone: text("phone"),
  requestedFareType: text("requested_fare_type")
    .notNull()
    .default("chilean"),
  effectiveFareType: text("effective_fare_type")
    .notNull()
    .default("chilean"),
  residenceVerificationStatus: text("residence_verification_status")
    .notNull()
    .default("not_required"),
  residenceRequestedAt: timestamp("residence_requested_at", {
    withTimezone: true,
  }),
  residenceReviewedAt: timestamp("residence_reviewed_at", {
    withTimezone: true,
  }),
  residenceReviewedBy: uuid("residence_reviewed_by").references(
    () => users.id,
    { onDelete: "set null" },
  ),
