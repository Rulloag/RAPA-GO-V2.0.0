    ...
    async function request<T>(
      ...
      // CORRECCIÓN: Si el "path" ya viene con "/api", limpiamos el duplicado eliminando la barra redundante.
      let cleanPath = path;
      if (BASE_URL.endsWith("/api") && path.startsWith("/api")) {
        cleanPath = path.substring(4); // Remueve los primeros 4 caracteres ("/api")
      }
