  ...
  async function fetchRapaGoFareRules(): Promise<RapaGoFareRules> {
    ...
    try {
      ...
      return {
        ...
        prontoPagaPercent: parseStoredNumber(
          parsed.prontoPagaPercent,
          fallback.prontoPagaPercent,
        ),
  ...
  function calculateRapaGoFare(
    ...
    const prontoPagaFare = roundByAdminRule(
      cashFare * (1 + rules.prontoPagaPercent / 100),
      rules,
    );
  ...
  type ProntoPagaCreatePaymentData = {
    urlPay?: string;
    uid?: string;
    reference?: string;
    order?: string;
  ...
  function getRequestRideProntoPagaApiBaseUrl(): string {
  ...
  function readRegistrationProfileValue(key: string): string {
    try {
      const raw = localStorage.getItem("rapago_registration_profile");
  ...
  function readFirstLocalValue(keys: string[]): string {
  ...
  function getRequestRideUserField(user: unknown, keys: string[]): string {
  ...
  function getProntoPagaClientData(user: unknown): {
    clientName: string;
    clientEmail: string;
    clientPhone: string;
    ...
    const storedName =
      readRegistrationProfileValue("name") ||
      `${readRegistrationProfileValue("firstName")} ${readRegistrationProfileValue("lastName")}`.trim();

    const clientName =
      getRequestRideUserField(user, ["name", "fullName"]) ||
      storedName ||
      "Cliente Rapa Go";

    const clientEmail =
      getRequestRideUserField(user, ["email"]) ||
      readRegistrationProfileValue("email") ||
      "cliente@rapago.cl";

    const clientPhone =
      getRequestRideUserField(user, ["phone", "phoneNumber"]) ||
      readRegistrationProfileValue("phone") ||
      readFirstLocalValue(["rapago_profile_phone", "phone"]) ||
      "56900000000";

    const clientDocument =
      getRequestRideUserField(user, ["rut", "document", "documentNumber"]) ||
      readRegistrationProfileValue("rut") ||
      readRegistrationProfileValue("document") ||
      readFirstLocalValue(["rapago_profile_rut", "rut", "clientDocument"]) ||
      "11111111-1";
  ...
  async function createProntoPagaPaymentForRide(input: {
    token: string;
    rideId: string;
    amountClp: number;
    user: unknown;
  }): Promise<ProntoPagaCreatePaymentData> {
    const client = getProntoPagaClientData(input.user);

    const response = await fetch(
      `${getRequestRideProntoPagaApiBaseUrl()}/payments/prontopaga/create`,
      {
        method: "POST",
        headers: {
    ...
    let parsed: { ok?: boolean; data?: ProntoPagaCreatePaymentData; message?: string; error?: string } & ProntoPagaCreatePaymentData = {};

    try {
      parsed = text ? (JSON.parse(text) as typeof parsed) : {};
    } catch {
      parsed = {
        message: text || `Respuesta inválida del backend (${response.status}).`,
      };
    }
    ...
    if (!response.ok || parsed.ok === false) {
      throw new Error(
        parsed.message ||
          parsed.error ||
          `No se pudo crear el pago ProntoPaga (${response.status}).`,
      );
    }

    return parsed.data ?? (parsed as ProntoPagaCreatePaymentData);
  }

  function getCreatedRideId(createdRide: unknown): string | null {
    if (!createdRide || typeof createdRide !== "object") return null;

    const data = createdRide as Record<string, unknown>;
    const candidates = [
      data.id,
      data.rideId,
      data.requestId,
      (data.data as Record<string, unknown> | undefined)?.id,
      (data.data as Record<string, unknown> | undefined)?.rideId,
    ];
  ...
  export default function RequestRidePage(): JSX.Element {
    ...
    function getPaymentLabel(method: PaymentMethod): string {
      ...
      if (method === "card") return fareQuote ? `Tarjeta ProntoPaga · ${formatCLP(fareQuote.prontoPagaFare)}` : "Tarjeta / ProntoPaga";
      return "Pendiente";
    }

    const cashPaymentLabel = fareQuote ? formatCLP(fareQuote.cashFare) : "Calculando";
    const cardPaymentLabel = fareQuote ? formatCLP(fareQuote.prontoPagaFare) : "Calculando";
    ...
    const cardPaymentUsdLabel = fareQuote
      ? formatUSDFromCLP(fareQuote.prontoPagaFare, fareQuote.usdRate)
      : "Calculando";
    ...
    function getSelectedFareAmount(method: PaymentMethod = paymentMethod): number | null {
      ...
      return method === "card" ? fareQuote.prontoPagaFare : fareQuote.cashFare;
    }
    ...
    async function handleRequest(methodOverride?: Exclude<PaymentMethod, null>): Promise<void> {
      const activePaymentMethod = methodOverride ?? paymentMethod;

      if (!session?.accessToken && activePaymentMethod !== "card") {
        setSubmitError("Tu sesión no está activa. Vuelve a iniciar sesión.");
        return;
      }
      ...
      if (!activePaymentMethod) {
        setShowPaymentBox(true);
        setSubmitError("Antes de solicitar el viaje debes elegir la forma de pago.");
        return;
      }
      ...
      try {
        ...
        if (selectedFareAmount != null) {
          ...
          (input as CreateRideInput & { paymentMethod?: string }).paymentMethod = activePaymentMethod === "card" ? "prontopaga_card" : "cash";
        }
         ...
         * Tu backend está devolviendo HTTP 500 en esa ruta y eso cortaba el flujo antes de abrir tarjeta.
        ...
        if (activePaymentMethod === "card") {
          ...
          const payment = await createProntoPagaPaymentForRide({
            token: session?.accessToken ?? "local-demo",
            rideId: localRideId,
          ...
          if (!payment.urlPay) {
            throw new Error("ProntoPaga no devolvió el enlace de pago urlPay.");
          }
        ...
        try {
          ...
          const backendMessage =
            backendErr instanceof Error
              ? backendErr.message
              : "El backend rechazó crear el viaje.";
          ...
          saveLocalPassengerRides([localRide, ...readLocalPassengerRides()]);
          console.warn("RAPA GO: backend rechazó crear viaje. Se guardó localmente.", backendMessage);
        }
        ...
        setSubmitError(
          `No se pudo abrir ProntoPaga: ${message}. Revisa que las rutas del backend estén en apps/api/src/modules/payments y no dentro de mobile/src/app.`,
        );
      } finally {
        setSubmitting(false);
      }
            ...
            >
              ...
              >
                ...
                {fareQuote && (
                  ...
                  >
                    <div>
                      <div style={{ fontSize: ".68rem", fontWeight: 950, color: "#7A5417", textTransform: "uppercase" }}>
                        Total efectivo
                      </div>
                      <div style={{ fontSize: "1rem", fontWeight: 950, color: "#111111", marginTop: 3 }}>
                        {formatCLP(fareQuote.cashFare)}
                      </div>
                      <div style={{ fontSize: ".74rem", fontWeight: 900, color: "rgba(17,17,17,.58)", marginTop: 2 }}>
                        {formatUSDFromCLP(fareQuote.cashFare, fareQuote.usdRate)}
                      </div>
                    </div>
                    <div>
                      ...
                      <div style={{ fontSize: "1rem", fontWeight: 950, color: "#111111", marginTop: 3 }}>
                        {formatCLP(fareQuote.prontoPagaFare)}
                      </div>
                      <div style={{ fontSize: ".74rem", fontWeight: 900, color: "rgba(17,17,17,.58)", marginTop: 2 }}>
                        {formatUSDFromCLP(fareQuote.prontoPagaFare, fareQuote.usdRate)}
                      </div>
                ...
                {showPaymentBox && (
                  ...
                  >
                    ...
                    <div style={{ color: "#5e4a22", fontSize: ".76rem", fontWeight: 800, lineHeight: 1.35, marginBottom: 12 }}>
                      Elige un método. Efectivo solicita el viaje; ProntoPaga abre el formulario seguro para completar tarjeta.
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      ...
                      <button
                        ...
                        style={{
                          border: paymentMethod === "card" ? "3px solid #ffffff" : "2px solid rgba(255,255,255,.45)",
                          borderRadius: "16px",
                          padding: "12px 8px",
                          minHeight: "82px",
                          background: "linear-gradient(180deg,#3B82F6 0%,#1D4ED8 100%)",
                          color: "#ffffff",
                          boxShadow: paymentMethod === "card" ? "0 12px 24px rgba(37,99,235,.36)" : "0 8px 18px rgba(0,0,0,.08)",
                      ...
                      >
                        <div style={{ fontSize: "1.35rem", lineHeight: 1 }}>💳</div>
                        <div style={{ marginTop: 5 }}>ProntoPaga</div>
                        <div style={{ marginTop: 4, fontSize: ".82rem", fontWeight: 950 }}>
                          {cardPaymentLabel}
                        </div>
                        <div style={{ marginTop: 2, fontSize: ".72rem", fontWeight: 850, opacity: .78 }}>
  ...
  <IonButton
              ...
              >
                {submitting ? <IonSpinner name="dots" /> : paymentMethod === "card" ? "PAGAR CON PRONTOPAGA" : "SOLICITAR VIAJE"}
              </IonButton>
            </div>
          </div>
