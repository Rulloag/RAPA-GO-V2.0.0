...
} from "../../features/rides/rides.service";
import {
  MapFallback,
...
import type {
  RentalVehicleData as RentalVehicleDataType,
  RentalBookingData as RentalBookingDataType,
...
function getRapaGoConnectivityMessage(
  ...
  if (status === "offline") {
    return "Sin internet: puedes revisar lo último cargado, pero para solicitar, pagar o cancelar viajes necesitas conexión. Tu sesión sigue activa: al volver la señal te reconectamos automáticamente.";
  }
...
const RAPAGO_PASSENGER_AUTH_SESSION_PROFILE_KEY =
  "rapago_registration_profile_session";
const RAPAGO_PASSENGER_RESIDENT_VERIFICATION_SESSION_KEY =
  "rapago_resident_verification_requests_session_v1";
...
function readPassengerSessionRegistrationProfile(): StoredRegistrationProfile {
  try {
    const raw = sessionStorage.getItem(
      RAPAGO_PASSENGER_AUTH_SESSION_PROFILE_KEY,
    );
...
function sanitizePassengerLocalRegistrationProfile(
  profile: StoredRegistrationProfile,
): StoredRegistrationProfile {
  return {
    passengerFareType:
      profile.passengerFareType ??
      profile.farePassengerType ??
    ...
    passengerFareLabel:
      profile.passengerFareLabel ?? profile.nationality ?? null,
  ...
  try {
    ...
    return {
      ...
      phone:
        ...
        passengerSessionValue("rapago_profile_phone") ||
        passengerSessionValue("rapago_passenger_phone"),
      passengerFareType:
...
function persistPassengerAutofill(
  ...
  try {
    ...
    const currentLocal = sanitizePassengerLocalRegistrationProfile(
      readStoredRegistrationProfile(),
    );
    const nextLocal = sanitizePassengerLocalRegistrationProfile({
      ...currentLocal,
      ...data,
    });
...
function readAdminFareUsdRate(): number {
  try {
    ...
    if (rawEngine) {
      const parsed = JSON.parse(rawEngine) as {
    ...
    const fromCards = Number(
      localStorage.getItem(ADMIN_FARE_USD_RATE_STORAGE_KEY),
    );
...
function formatUsdFromClp(
  value: number | null | undefined,
  usdRate = readAdminFareUsdRate(),
): string {
...
const DEFAULT_PASSENGER_FARE_RULES: AdminFareCardRule[] = [
  ...
  },
  {
    ...
    fixedClp: null,
    description:
      "Chileno no residente · vehículo con espacio extra para maletas.",
    active: true,
  },
  {
  ...
  try {
    ...
    return {
      urban: {
        ...
        baseMinimumClp: Number(
          parsed.urban?.baseMinimumClp ?? base.urban.baseMinimumClp,
        ),
      ...
      passengerMultipliers: {
        resident: Number(
          parsed.passengerMultipliers?.resident ??
            base.passengerMultipliers.resident,
        ),
        chilean: Number(
          parsed.passengerMultipliers?.chilean ??
            base.passengerMultipliers.chilean,
        ),
        foreigner: Number(
          parsed.passengerMultipliers?.foreigner ??
            base.passengerMultipliers.foreigner,
        ),
      },
      vehicleMultipliers: {
        standard: Number(
          parsed.vehicleMultipliers?.standard ??
            base.vehicleMultipliers.standard,
        ),
        xl: Number(parsed.vehicleMultipliers?.xl ?? base.vehicleMultipliers.xl),
        luggage: Number(
          parsed.vehicleMultipliers?.luggage ?? base.vehicleMultipliers.luggage,
        ),
      },
      fixedDestinations:
        ...
        parsed.fixedDestinations.length > 0
          ? parsed.fixedDestinations.map((item, index) => ({
              id: normalizeFareSearchText(
                String(item.id ?? item.title ?? `destino_${index + 1}`),
              ).replace(/[^a-z0-9]+/g, "_"),
      ...
      rounding: {
        mode:
          parsed.rounding?.mode === "nearest" ||
          parsed.rounding?.mode === "none"
            ? parsed.rounding.mode
            : "ceil",
        unitClp: Math.max(
          1,
          Number(parsed.rounding?.unitClp ?? base.rounding.unitClp),
        ),
      },
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      ...
      for (const fallback of DEFAULT_PASSENGER_FARE_RULES) {
        if (!merged.some((rule) => rule.id === fallback.id))
...
const RESIDENT_VERIFICATION_REQUESTS_KEY =
  "rapago_resident_verification_requests_v1";
...
function normalizeResidenceVerificationStatus(
  value: unknown,
...
function readResidentVerificationRequests(): Array<Record<string, unknown>> {
  ...
  try {
    const sessionRaw = sessionStorage.getItem(
      RAPAGO_PASSENGER_RESIDENT_VERIFICATION_SESSION_KEY,
    );
    const sessionParsed = sessionRaw
      ? (JSON.parse(sessionRaw) as Array<Record<string, unknown>>)
      : [];
...
function readResidentVerificationState(
  user?: unknown,
  ...
  const statusFromRequest = normalizeResidenceVerificationStatus(
    request?.status,
  );
  const statusFromUser =
    normalizeResidenceVerificationStatus(
      getUserStringField(user, "residenceVerificationStatus"),
    ) ??
    normalizeResidenceVerificationStatus(
      getUserStringField(user, "residentVerificationStatus"),
    ) ??
    normalizeResidenceVerificationStatus(
      getUserStringField(user, "rapaNuiVerificationStatus"),
    );
  ...
  const documentName = String(
    request?.documentName ?? stored.residentDocumentName ?? "",
  );

  const rejectionReason = String(
    request?.rejectionReason ?? request?.reason ?? request?.reviewMessage ?? "",
  ).trim();
...
function setPassengerTypeAfterResidentRejection(
  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent("rapago:resident-verification-updated"),
    );
  } catch {
    // No bloquea la app.
  }
...
function findActiveFareRule(
  rules: AdminFareCardRule[],
  id: string,
): AdminFareCardRule | null {
...
function calculateRapaGoFareFromEngine(
  ...
  const fixedDestination = detectEngineFixedDestination(
    config,
  ...
  if (fixedDestination) {
    const exactFixedFare =
      fixedDestination.baseResidentClp * passengerMultiplier;
...
function calculateRapaGoFareFromCompatibilityRules(
  ...
  if (
    specificVariableRule?.minimumClp != null ||
    specificVariableRule?.kmClp != null
  ) {
    const minimumFare = Math.max(
      0,
      Number(specificVariableRule.minimumClp ?? 5000),
    );
...
function passengerInputItemStyle(
...
export function PassengerHomePage(): JSX.Element {
  ...
  const [residentVerificationRevision, setResidentVerificationRevision] =
    useState(0);
  ...
  useEffect(() => {
    ...
    void passengerProfileService
      ...
      .then((data) => {
        const storedPhone = getAutoPhone(
  ...
  const initials =
    name
      ...
      .join("")
      .toUpperCase() || "P";
  const phoneFromRegister = getAutoPhone(
    getSessionPhone(session?.user),
    profile?.phone,
  );
  ...
  return (
    <IonPage>
      ...
      <div
        style={{
          background:
            "linear-gradient(145deg, var(--ion-color-primary) 0%, var(--ion-color-primary-shade) 100%)",
...
function buildRideScheduleFields(input: {
  ...
  return {
    ...
    scheduleStatus:
      input.rideMode === "scheduled" && !!pickup
        ? "pending_activation"
        : "immediate",
...
function upsertLocalAdminScheduledRide(ride: RideRequestData): void {
  ...
  const withoutDuplicate = readLocalAdminScheduledRides().filter(
    (item) =>
      getScheduledRideKey(item as unknown as Record<string, unknown>) !== key,
  );
...
function RequestRidePage(): JSX.Element {
  ...
  const [vehicleCategory, setVehicleCategory] =
    useState<VehicleFareCategory>("standard");
  ...
  const [blockingRide, setBlockingRide] = useState<RideRequestData | null>(() =>
    findBlockingPassengerRide(),
  );

  useEffect(() => {
    const refreshBlockingRide = () =>
      setBlockingRide(findBlockingPassengerRide());
    refreshBlockingRide();
    window.addEventListener("storage", refreshBlockingRide);
    window.addEventListener(
      "rapago:passenger-rides-updated",
      refreshBlockingRide as EventListener,
    );
  ...
  useEffect(() => {
    ...
    const destinationPoint = selectedDestId
      ? getCoordsFromPlaceId(selectedDestId)
      : null;
    ...
    const destName = selectedDestId
      ? (RAPA_NUI_PLACES.find((p) => p.id === selectedDestId)?.name ??
        destInput)
      : destInput;
  ...
  async function handleRequest() {
    ...
    if (currentBlockingRide) {
      ...
      setSubmitError(
        "Ya tienes un viaje o una reserva activa. Revísala en Mis Viajes antes de solicitar otra.",
      );
      return;
    }
    ...
    if (!farePreview) {
      setSubmitError(
        "No se pudo calcular la tarifa. Selecciona origen, destino y confirma el punto.",
      );
      return;
    }
    ...
    if (paymentMethod === "card") {
      setSubmitError(
        "El pago con tarjeta estará disponible próximamente. Por ahora selecciona efectivo.",
      );
      return;
    }
    ...
    (
      input as unknown as { estimatedFareClp?: number; paymentMethod?: string }
    ).estimatedFareClp =
      tripFareMode === "round_trip" ? farePreview.fare * 2 : farePreview.fare;
    (
      input as unknown as { estimatedFareClp?: number; paymentMethod?: string }
    ).paymentMethod = "cash";
    (
    ...
    ).nationality = passengerFareTypeLabel(passengerFareType);
    (
      input as unknown as {
        passengerFareLabel?: string;
        nationality?: string;
    ...
    (
      input as unknown as { requestedByRole?: string; requesterRole?: string }
    ...
    (
      input as unknown as {
        fareVehicleCategory?: VehicleFareCategory;
        vehicleCategory?: VehicleFareCategory;
      }
    ...
    ).isRoundTrip = tripFareMode === "round_trip";
    Object.assign(
      input as unknown as Record<string, unknown>,
      buildRideScheduleFields({
        rideMode,
        tripFareMode,
        scheduledAt,
        returnScheduledAt,
    ...
    const selectedFare =
      tripFareMode === "round_trip" ? farePreview.fare * 2 : farePreview.fare;
    const selectedDriverEarnings =
      tripFareMode === "round_trip"
        ? farePreview.driverEarnings * 2
        : farePreview.driverEarnings;

    const notes: string[] = [];

    if (currentLat !== null && currentLng !== null) {
      notes.push(
        `Ubicación real del pasajero: ${currentLat.toFixed(6)}, ${currentLng.toFixed(6)}.`,
      );
      notes.push(
        `Coordenadas recogida accesible: ${currentLat.toFixed(6)}, ${currentLng.toFixed(6)}.`,
      );
    } else if (originCoords.lat != null && originCoords.lng != null) {
      notes.push(
        `Coordenadas recogida accesible: ${Number(originCoords.lat).toFixed(6)}, ${Number(originCoords.lng).toFixed(6)}.`,
      );
    }

    if (destCoords.lat != null && destCoords.lng != null) {
      notes.push(
        `Coordenadas destino accesible: ${Number(destCoords.lat).toFixed(6)}, ${Number(destCoords.lng).toFixed(6)}.`,
      );
    }
    ...
    notes.push(
      `Tipo de pasajero tarifario: ${passengerFareTypeLabel(passengerFareType)}.`,
    );
    notes.push(
      `Tipo de viaje seleccionado: ${tripFareModeLabel(tripFareMode)}.`,
    );
    ...
    try {
      ...
      if (rideMode === "scheduled") {
        upsertLocalAdminScheduledRide({
          ...
          passengerName:
            getUserDisplayName(session?.user) ?? "Pasajero agendado",
  ...
  const originCoords = selectedOriginId
    ? getPlaceCoordinates(selectedOriginId)
    : {};
  ...
  const fareDisplayAmount = farePreview
    ? tripFareMode === "round_trip"
      ? farePreview.fare * 2
      : farePreview.fare
    : null;
          ...
          {blockingRide && (
            <IonCard
              ...
              <IonCardContent style={{ padding: "14px 16px" }}>
                <div
                  style={{ fontWeight: 950, color: "#111", marginBottom: 4 }}>
                ...
                {getPassengerRideScheduleInfo(blockingRide).isScheduled && (
                  <div
                    style={{ fontSize: ".78rem", color: "#555", marginTop: 4 }}>
                    Recogida:{" "}
                    {formatScheduleDateTime(
                      getPassengerRideScheduleInfo(blockingRide).scheduledAt,
                    )}
                  </div>
                )}
                <IonButton
                ...
                <div
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div
                      ...
                      En sectores con pasajes, condominios o calles interiores,
                      Rapa Go recomendará una calle principal accesible para el
                      conductor.
                    </div>
                  </div>
                </div>
            ...
            <IonCardContent style={{ padding: "16px" }}>
              ...
              <IonItem lines="full" style={passengerInputItemStyle()}>
                ...
                <IonInput
                  ...
                  onIonInput={(e) => {
                    setOriginInput(
                      sanitizeText(String(e.detail.value ?? ""), 150),
                    );
                    setSelectedOriginId("");
                    setPickupConfirmed(false);
                  }}
              ...
              {rideMode === "scheduled" && (
                <div
                  ...
                  <IonItem lines="full" style={passengerInputItemStyle()}>
                    ...
                    <IonInput
                      ...
                      max={toDateTimeLocalValue(
                        new Date(
                          Date.now() + SCHEDULE_MAX_DAYS * 24 * 60 * 60_000,
                        ),
                      )}
                      onIonInput={(e) =>
                        setScheduledAt(String(e.detail.value ?? ""))
                      }
                    />
                  </IonItem>
                      ...
                      <IonInput
                        ...
                        min={
                          ...
                          toDateTimeLocalValue(
                            new Date(
                              Date.now() + SCHEDULE_MIN_MINUTES * 60_000,
                            ),
                          )
                        }
                        max={toDateTimeLocalValue(
                        ...
                        onIonInput={(e) =>
                          setReturnScheduledAt(String(e.detail.value ?? ""))
                        }
                      />
                    </IonItem>
                  )}
                  ...
                  <IonNote
                    ...
                    Se guarda en Mis Viajes y se activa para buscar conductores{" "}
                    {SCHEDULE_ACTIVATION_MINUTES} minutos antes de la reserva.
                  </IonNote>
                </div>
              )}
              ...
              <IonItem lines="none" style={passengerInputItemStyle()}>
                ...
                <IonTextarea
                  ...
                  onIonInput={(e) =>
                    setNotesInput(
                      sanitizeText(String(e.detail.value ?? ""), 500),
                    )
                  }
              ...
              {farePreview && (
                <div
                  style={{
                    ...
                    padding: "14px",
                    background:
                      "linear-gradient(135deg,#fffdf8 0%,#f8f1df 48%,#ecd29a 100%)",
                  ...
                  <div
                    ...
                    <div>
                      ...
                      <div
                        ...
                        {formatUsdFromClp(
                          fareDisplayAmount ?? farePreview.fare,
                        )}
                      </div>
                      <div
                        style={{
                        ...
                        {farePreview.km.toFixed(1)} km · {farePreview.minutes}{" "}
                        min ·{" "}
                        {farePreview.isZoneFare
                          ? "tarifa fija"
                          : "precio calculado"}
                      </div>
                      <div
                        ...
                        Tarifa pasajero:{" "}
                        {passengerFareTypeLabel(passengerFareType)}
                      </div>
                    </div>
                  ...
                  <div
                    ...
                    <button
                      ...
                      style={{
                        ...
                        boxShadow:
                          paymentMethod === "cash"
                            ? "0 0 22px rgba(212,166,42,.55)"
                            : "0 8px 18px rgba(0,0,0,.10)",
                        transform:
                          paymentMethod === "cash" ? "scale(1.02)" : "scale(1)",
                        fontWeight: 950,
                      }}>
                      <div style={{ fontSize: "1.2rem" }}>💵</div>
                      <div>Efectivo</div>
                      <div style={{ fontSize: ".8rem", marginTop: "3px" }}>
                        {formatClp(fareDisplayAmount ?? farePreview.fare)}
                      </div>
                      <div
                        style={{
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSubmitError(
                          "El pago con tarjeta estará disponible próximamente. Por ahora puedes solicitar el viaje en efectivo.",
                        );
                      }}
                      style={{
                        border:
                        ...
                        boxShadow:
                          paymentMethod === "card"
                            ? "0 0 18px rgba(120,120,120,.35)"
                            : "0 8px 18px rgba(0,0,0,.08)",
                        transform:
                          paymentMethod === "card" ? "scale(1.02)" : "scale(1)",
                        fontWeight: 950,
                        position: "relative",
                        overflow: "hidden",
                      }}>
                      <div style={{ fontSize: "1.2rem" }}>💳</div>
                      <div>Tarjeta</div>
                      <div style={{ fontSize: ".78rem", marginTop: "3px" }}>
                        Próximamente
                      </div>
                      <div
                        style={{
              ...
              <IonButton
                ...
                disabled={
                  ...
                  (rideMode === "scheduled" &&
                    (!scheduledAt ||
                      (tripFareMode === "round_trip" && !returnScheduledAt)))
                }>
                {submitting ? (
                  <IonSpinner name="dots" />
                ) : passengerConnection.blocked ? (
              ...
              {!pickupConfirmed && (
                <IonNote
                  ...
                  Debes confirmar el punto de partida antes de solicitar el
                  viaje.
                </IonNote>
              )}
            </IonCardContent>
...
function readPassengerRequeuedRides(): RideRequestData[] {
  try {
    ...
    for (const key of [
      ...
      const parsed = raw
        ? (JSON.parse(raw) as Array<RideRequestData & Record<string, unknown>>)
        : [];
    ...
    for (const ride of all) {
      const id = String(
        ride.originalRideId ?? ride.rideId ?? ride.id ?? "",
      ).trim();
      const routeKey = [
    ...
    return Array.from(byRide.values()).map((ride) => ({
      ...
      id: String(
        (ride as Record<string, unknown>).id ??
...
function isPassengerDriverCancelledRequeue(ride: RideRequestData): boolean {
  ...
  const cancelledBy = String(
    record.cancelledByRole ?? record.cancelledBy ?? "",
  ).toLowerCase();
  const reason = String(
    record.requeuedReason ??
      record.requeueReason ??
      record.cancellationReason ??
      "",
  ).toLowerCase();
  const notice = String(
    record.passengerNotice ??
      record.passengerNotification ??
      record.notes ??
      "",
  ).toLowerCase();
...
function syncRequeuedRidesIntoLocalPassengerRides(): RideRequestData[] {
  ...
  const next = mergePassengerRidesForDisplay([...current, ...requeued]).slice(
    0,
    200,
  );
...
function getPassengerRideStorageKey(
  ride: Partial<RideRequestData> & Record<string, unknown>,
): string {
  const scheduledAt =
    ride.scheduledAt ??
  ...
  return [
    ride.id?.startsWith?.("local-") || ride.id?.startsWith?.("admin-local-")
      ? "local-mirror"
      : ride.id,
    scheduledAt,
    ride.originText ?? "",
...
function getPassengerCancelIds(
  ride: Partial<RideRequestData> & Record<string, unknown>,
): Set<string> {
  return new Set(
...
function getPassengerCancelRouteKey(
  ride: Partial<RideRequestData> & Record<string, unknown>,
): string {
  return [
...
function getPassengerCancelScheduleKey(
  ride: Partial<RideRequestData> & Record<string, unknown>,
): string {
  return String(
    ride.scheduledAt ??
...
function isSamePassengerRideCancelTarget(
  ...
  if (!candidateRoute || !targetRoute || candidateRoute !== targetRoute)
    return false;
  ...
  if (
    candidateSchedule &&
    targetSchedule &&
    candidateSchedule === targetSchedule
  )
    return true;
...
function isPassengerRideCancelledByPassenger(
  ride: Partial<RideRequestData> & Record<string, unknown>,
): boolean {
  const status = String(ride.status ?? "").toLowerCase();
  const cancelledBy = normalizePassengerCancelText(
    ride.cancelledByRole ?? ride.cancelledBy,
  );
  const reason = normalizePassengerCancelText(
    ride.cancellationReason ?? ride.cancelReason ?? ride.notes,
  );
...
function hasPassengerCancelledRideMarker(
  target: Partial<RideRequestData> & Record<string, unknown>,
): boolean {
  try {
    return readLocalPassengerRides().some(
      (ride) =>
        isPassengerRideCancelledByPassenger(
          ride as RideRequestData & Record<string, unknown>,
        ) &&
...
function removePassengerRideFromArrayStorage(
  ...
  try {
    ...
    const next = parsed.filter(
      (ride) => !isSamePassengerRideCancelTarget(ride, target),
    );
...
function cancelPassengerRideEverywhere(
  ...
  for (const key of PASSENGER_CANCEL_STORAGE_KEYS) {
    if (key !== "rapago_local_passenger_rides") {
      removePassengerRideFromArrayStorage(
        key,
        cancelled as RideRequestData & Record<string, unknown>,
      );
    }
  }
  ...
  window.dispatchEvent(
    new CustomEvent(RAPAGO_REQUEUED_RIDES_EVENT, { detail: { cancelled } }),
  );
...
function applyPassengerCancelledRideToList(
  ...
  const next = rides.map((ride) => {
    if (
      !isSamePassengerRideCancelTarget(
        ride as RideRequestData & Record<string, unknown>,
        target as RideRequestData & Record<string, unknown>,
      )
    ) {
      return ride;
    }
...
function upsertLocalPassengerRideMirror(ride: RideRequestData): void {
  const key = getPassengerRideStorageKey(
    ride as Partial<RideRequestData> & Record<string, unknown>,
  );
  const current = readLocalPassengerRides();
  const withoutDuplicate = current.filter((item) => {
    const itemKey = getPassengerRideStorageKey(
      item as Partial<RideRequestData> & Record<string, unknown>,
    );
...
function cleanPassengerNotes(notes: string | null | undefined): string | null {
  ...
  return (
    notes
      ...
      .replace(
        /Ubicación GPS pasajero:.*?(?=Punto de partida confirmado|$)/i,
        "",
      )
      .replace(
        /Ubicación real del pasajero:.*?(?=Punto accesible|Coordenadas|$)/i,
        "",
      )
      .replace(
...
function getRidePassengerFareType(
...
function passengerIndexFlattenObjects(
  value: unknown,
  depth = 0,
  ...
  if (Array.isArray(value))
    return value.flatMap((item) =>
      passengerIndexFlattenObjects(item, depth + 1),
    );
...
function passengerIndexParseVehicleLabel(
  label: unknown,
): Partial<PassengerIndexVehicleDisplayData> {
  const clean = passengerIndexStringValue(label)
    .replace(
      /\b(propio|prestado|borrowed|own|activo|veh[ií]culo activo)\b/gi,
      "",
    )
    .replace(/\s{2,}/g, " ")
    .trim();
  ...
  parsed.plate =
    parts
      ...
      .find(
        (part) =>
          /(?=.*\d)[A-Z0-9-]{3,}/i.test(part) &&
          !/color|propio|prestado/i.test(part),
      ) ??
    clean
      .split(/\s+/)
      .find(
...
function passengerIndexFindImageInObject(
...
function passengerIndexIsFareRuleObject(
  obj: Record<string, unknown>,
  sourceKey = "",
): boolean {
  ...
  if (
    ...
    "kmClp" in obj ||
    "fixedClp" in obj ||
    "baseResidentClp" in obj
  )
    return true;
  if (
    title.includes("tarifa") ||
...
function passengerIndexBuildVehicleCandidate(
  obj: Record<string, unknown>,
  sourceKey = "",
): PassengerIndexVehicleDisplayData | null {
  ...
  if (
    ...
    lowerKey.includes("selected")
  )
    score += 35;
  if (lowerKey.includes("driver_active_vehicle")) score += 45;
  if (lowerKey.includes("current_driver") || lowerKey.includes("live"))
    score += 12;

  const updatedTime = new Date(
    String(obj.updatedAt ?? obj.createdAt ?? obj.selectedAt ?? ""),
  ).getTime();
...
function passengerIndexCandidateFromString(
  value: unknown,
  sourceKey = "",
): PassengerIndexVehicleDisplayData | null {
  ...
  try {
    ...
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      if (
        ...
        continue;
      if (
        !/(vehicle|vehiculo|driver|conductor|auto|(^|[_-])car([_-]|$))/i.test(
          lower,
        )
      )
        continue;
...
function passengerIndexReadBestVehicleFromEverywhere(): PassengerIndexVehicleDisplayData | null {
  ...
  try {
    ...
    for (const key of keys) {
      ...
      const fromString =
        typeof value === "string"
          ? passengerIndexCandidateFromString(value, key)
          : null;
      ...
      for (const obj of passengerIndexFlattenObjects(value)) {
        ...
        const ownership = passengerIndexFirstValue(
          obj.ownership,
          obj.driverVehicleOwnership,
        ).toLowerCase();
...
function passengerIndexGetDriverVehiclePublicData(
  ride: RideRequestData & Record<string, unknown>,
): PassengerIndexVehicleDisplayData {
  const fallback = passengerIndexReadBestVehicleFromEverywhere();

  return {
    brand: passengerIndexFirstValue(
      ride.driverVehicleBrand,
      ride.vehicleBrand,
      fallback?.brand,
    ),
    model: passengerIndexFirstValue(
      ride.driverVehicleModel,
      ride.vehicleModel,
      fallback?.model,
    ),
    color: passengerIndexFirstValue(
      ride.driverVehicleColor,
      ride.vehicleColor,
      fallback?.color,
    ),
    plate: passengerIndexFirstValue(
...
function passengerIndexGetDriverName(
  ride: RideRequestData & Record<string, unknown>,
): string {
  ...
  try {
    for (const key of [
      ...
      for (const obj of passengerIndexFlattenObjects(value)) {
        const name = passengerIndexFirstValue(
          obj.driverFullName,
          obj.driverName,
          obj.fullName,
          obj.name,
...
function enrichPassengerRideWithDriverVehicleData(
  ride: RideRequestData,
): RideRequestData {
  const record = ride as RideRequestData & Record<string, unknown>;
  ...
  return {
    ...record,
    driverName:
      passengerIndexFirstValue(record.driverName, driverName) ||
      record.driverName ||
      null,
    driverFullName:
      passengerIndexFirstValue(record.driverFullName, driverName) ||
      record.driverFullName ||
      null,
    driverVehicleBrand:
      passengerIndexFirstValue(record.driverVehicleBrand, vehicle.brand) ||
      null,
    driverVehicleModel:
      passengerIndexFirstValue(record.driverVehicleModel, vehicle.model) ||
      null,
    driverVehicleColor:
      passengerIndexFirstValue(record.driverVehicleColor, vehicle.color) ||
      null,
    driverVehiclePlate:
      passengerIndexFirstValue(record.driverVehiclePlate, vehicle.plate) ||
      null,
    driverVehicleImageDataUrl: passengerIndexImageValue(
...
function getRideScheduleString(
  ride: RideRequestData,
  keys: string[],
): string | null {
  for (const key of keys) {
...
function extractPassengerIsoByKeywords(
  ...
  for (const keyword of keywords) {
    ...
    const match = notes.match(
      new RegExp(`${safeKeyword}\\s*[:=]\\s*${isoPattern}`, "i"),
    );
...
function extractPassengerScheduleIsoFromNotes(
  notes: string | null | undefined,
): string | null {
...
function extractPassengerReturnIsoFromNotes(
  notes: string | null | undefined,
): string | null {
  return extractPassengerIsoByKeywords(notes, [
...
function getPassengerRideScheduleInfo(
  ride: RideRequestData,
): PassengerRideScheduleInfo {
  const scheduledAt =
    toPassengerIso(
      getRideScheduleString(ride, [
        "scheduledAt",
        "scheduledPickupAt",
        "pickupScheduledAt",
        "pickupAt",
    ...
    (scheduledAt
      ? new Date(
          new Date(scheduledAt).getTime() -
            SCHEDULE_ACTIVATION_MINUTES * 60_000,
        ).toISOString()
      : null);
  ...
  const isActiveWindow =
    !!activationAt && Date.now() >= new Date(activationAt).getTime();
...
function getEffectivePassengerRideStatus(ride: RideRequestData): string {
  ...
  if (ride.status === "scheduled" && schedule.isActiveWindow)
    return "requested";
  return ride.status;
}
...
function mergePassengerRidesForDisplay(
  rides: RideRequestData[],
): RideRequestData[] {
  ...
  for (const ride of rides) {
    ...
    const incomingIsLocal =
      ride.id.startsWith("local-") || ride.id.startsWith("admin-local-");
    const currentIsLocal = Boolean(
      current?.id.startsWith("local-") ||
      current?.id.startsWith("admin-local-"),
    );
    ...
    const incomingIsPassengerCancelled = isPassengerRideCancelledByPassenger(
      ride as RideRequestData & Record<string, unknown>,
    );
    const currentIsPassengerCancelled = isPassengerRideCancelledByPassenger(
      current as RideRequestData & Record<string, unknown>,
    );
    ...
    if (
      incomingIsDriverRequeued &&
      currentEffectiveStatus === "cancelled" &&
      !currentIsPassengerCancelled
    ) {
      byKey.set(key, {
        ...current,
        ...ride,
    ...
    if (
      currentIsDriverRequeued &&
      incomingEffectiveStatus === "cancelled" &&
      !incomingIsPassengerCancelled
    ) {
      continue;
    }
    ...
    if (
      incomingIsLocal &&
      !currentIsLocal &&
      getPassengerRideScheduleInfo(ride).isScheduled
    ) {
  ...
  return Array.from(byKey.values()).map(
    enrichPassengerRideWithDriverVehicleData,
  );
}

function sortPassengerRidesForDisplay(
  rides: RideRequestData[],
): RideRequestData[] {
  return [...rides].sort((a, b) => {
    const priorityDiff =
      getPassengerRidePriority(a) - getPassengerRidePriority(b);
...
function findBlockingPassengerRide(): RideRequestData | null {
  ...
  const rides = sortPassengerRidesForDisplay(
    mergePassengerRidesForDisplay([...readLocalPassengerRides(), ...requeued]),
  );
  return (
    rides.find((ride) =>
      PASSENGER_ACTIVE_STATUSES.includes(getEffectivePassengerRideStatus(ride)),
    ) ?? null
  );
}
...
function extractPassengerLiveNavPoints(
  notes: string | null | undefined,
): PassengerLiveNavPoints {
  return {
    pickupLat: extractLiveNumber(
      notes,
      /Coordenadas recogida accesible:\s*(-?\d+(?:[.,]\d+)?)/i,
    ),
    pickupLng: extractLiveNumber(
      notes,
      /Coordenadas recogida accesible:\s*-?\d+(?:[.,]\d+)?,\s*(-?\d+(?:[.,]\d+)?)/i,
    ),
    destinationLat: extractLiveNumber(
      notes,
      /Coordenadas destino accesible:\s*(-?\d+(?:[.,]\d+)?)/i,
    ),
    destinationLng: extractLiveNumber(
      notes,
      /Coordenadas destino accesible:\s*-?\d+(?:[.,]\d+)?,\s*(-?\d+(?:[.,]\d+)?)/i,
    ),
    passengerOriginalLat: extractLiveNumber(
      notes,
      /Ubicación real del pasajero:\s*(-?\d+(?:[.,]\d+)?)/i,
    ),
    passengerOriginalLng: extractLiveNumber(
      notes,
...
function PassengerDriverLiveMap({
  ...
  const [livePoint, setLivePoint] = useState<PassengerLiveDriverPoint | null>(
    null,
  );
  const [mapReady, setMapReady] = useState(false);
  const [liveMessage, setLiveMessage] = useState(
    "Esperando GPS real del conductor...",
  );
  ...
  const driverPoint =
    livePoint &&
    Number.isFinite(livePoint.lat) &&
    Number.isFinite(livePoint.lng)
    ...
    <div
      ...
      {!driverPoint && (
        <div
          ...
          El conductor debe tener la ubicación activa para que veas el punto
          azul moverse en tiempo real.
        </div>
      )}
    </div>
  );
...
function TripsPage(): JSX.Element {
  ...
  const loadRides = useCallback(async () => {
    ...
    try {
      ...
      if (passengerConnection.blocked || !session?.accessToken) {
        setAllRides(
          sortPassengerRidesForDisplay(
            mergePassengerRidesForDisplay([...localRides, ...requeuedRides]),
          ),
        );
        setPage(1);
        return;
  ...
  async function handleCancel(rideId: string) {
    ...
    setAllRides((prev) =>
      applyPassengerCancelledRideToList(prev, target, cancelledLocal),
    );
  ...
  async function handleSubmitRating() {
    ...
    try {
      ...
      setRatingError(
        safePassengerErrorMessage(
          err instanceof Error ? err.message : "Error al calificar el viaje.",
        ),
      );
    } finally {
  ...
  const counts = {
    all: allRides.length,
    active: allRides.filter((r) =>
      ACTIVE_STATUSES.includes(getEffectivePassengerRideStatus(r)),
    ).length,
    completed: allRides.filter(
      (r) => getEffectivePassengerRideStatus(r) === "completed",
    ).length,
    cancelled: allRides.filter(
      (r) => getEffectivePassengerRideStatus(r) === "cancelled",
    ).length,
  };
    ...
    <IonPage>
      ...
      <IonContent>
        <IonRefresher
          slot="fixed"
          onIonRefresh={(e) => {
            void loadRides().then(() => e.detail.complete());
          }}>
          <IonRefresherContent />
        </IonRefresher>
        ...
        {!loading && filtered.length > 0 && (
          <div
            ...
            {filtered.map((ride) => {
              ...
              const label =
                RIDE_STATUS_LABEL[effectiveStatus] ?? effectiveStatus;
              const isActive = ACTIVE_STATUSES.includes(effectiveStatus);
              const driverDisplayRide =
                enrichPassengerRideWithDriverVehicleData(ride);
              const driverDisplayRecord = driverDisplayRide as RideRequestData &
                Record<string, unknown>;
              const driverDisplayName = passengerIndexFirstValue(
                driverDisplayRecord.driverName,
                driverDisplayRecord.driverFullName,
              );
              const driverDisplayVehicle =
                passengerIndexGetDriverVehiclePublicData(driverDisplayRecord);
              ...
              const timelineSteps = [
                {
                  ...
                  active: effectiveStatus === "requested",
                },
                {
                  status: "accepted",
                  ...
                  active: effectiveStatus === "accepted",
                },
                {
                  status: "driver_en_route",
                  label: "Conductor en camino",
                  time: ride.enRouteAt,
                  completed: !!ride.enRouteAt,
                  active: effectiveStatus === "driver_en_route",
                },
                {
                  status: "driver_arrived",
                  label: "Conductor llegó",
                  time: ride.arrivedAt,
                  completed: !!ride.arrivedAt,
                  active: effectiveStatus === "driver_arrived",
                },
                {
                  status: "in_progress",
                  ...
                  active: effectiveStatus === "in_progress",
                },
                {
                  status: "completed",
                  label: "Completado",
              ...
              return (
                <IonCard
                  key={ride.id}
                  style={passengerCardStyle({ borderRadius: "20px" })}>
                  {/* Status bar */}
                  <div
                    style={{
                      height: "4px",
                  ...
                  <IonCardContent style={{ padding: "14px 16px" }}>
                    ...
                    <div
                      ...
                      <div
                        ...
                        <IonBadge color={color} style={{ fontSize: "0.7rem" }}>
                          {label}
                        </IonBadge>
                    ...
                    {scheduleInfo.isScheduled && (
                      <div
                        style={{
                          ...
                          padding: "12px",
                          borderRadius: "16px",
                          background: scheduleInfo.isActiveWindow
                            ? "rgba(42,168,74,.12)"
                            : "rgba(255,201,40,.16)",
                          border: scheduleInfo.isActiveWindow
                            ? "1px solid rgba(42,168,74,.30)"
                            : "1px solid rgba(255,201,40,.40)",
                          color: "#111",
                          fontSize: "0.78rem",
                          lineHeight: 1.35,
                        }}>
                        ...
                        <div>
                          Recogida:{" "}
                          {formatScheduleDateTime(scheduleInfo.scheduledAt)}
                        </div>
                        ...
                        <div>
                          {scheduleInfo.isActiveWindow
                            ? "Ya se activó para buscar conductor."
                            : `Se activará ${SCHEDULE_ACTIVATION_MINUTES} min antes de la hora reservada.`}
                        </div>
                      </div>
                    )}
                    ...
                    {scheduleInfo.isScheduled &&
                      ...
                      ride.driverName && (
                        <div
                          ...
                          <div>
                            {getPassengerRideNotification(ride) ??
                              `Tu conductor ${ride.driverName} fue agendado para este viaje.`}
                          </div>
                          <div style={{ marginTop: 4 }}>
                            El viaje se mantiene reservado y se activa
                            automáticamente 10 minutos antes.
                          </div>
                        </div>
                      )}
                      ...
                      <div
                        ...
                        {getRidePassengerFareType(ride) && (
                          <IonBadge
                            ...
                            {passengerFareTypeLabel(
                              getRidePassengerFareType(
                                ride,
                              ) as PassengerFareType,
                            )}
                          </IonBadge>
                        )}
                        {ride.discountApplied &&
                          ride.originalFareClp != null && (
                            <>
                              <IonBadge
                                color="success"
                                style={{ fontSize: "0.65rem" }}>
                      ...
                      ([
                        ...
                        <div style={{ marginBottom: "10px" }}>
                          <DriverInfoCard
                            ...
                            vehicleBrand={
                              driverDisplayVehicle.brand ||
                              driverDisplayRide.driverVehicleBrand
                            }
                            vehicleModel={
                              driverDisplayVehicle.model ||
                              driverDisplayRide.driverVehicleModel
                            }
                            vehicleColor={
                              driverDisplayVehicle.color ||
                              driverDisplayRide.driverVehicleColor
                            }
                            vehiclePlate={
                              driverDisplayVehicle.plate ||
                              driverDisplayRide.driverVehiclePlate
                            }
                    ...
                    {((isActive && effectiveStatus !== "scheduled") ||
                      ...
                      <div
                        ...
                        {ride.cancelledByRole && (
                          <div
                            ...
                            Cancelado por:{" "}
                            {ride.cancelledByRole === "passenger"
                              ? "pasajero"
                              : "conductor"}
                          </div>
                        )}
                    ...
                    <div
                      ...
                      {(effectiveStatus === "scheduled" ||
                        ...
                        <IonButton
                          ...
                          onClick={() =>
                            void (effectiveStatus === "scheduled" ||
                            effectiveStatus === "requested"
                              ? handleCancel(ride.id)
                              : handleCancelAccepted(ride.id))
                          }>
                          {cancelling === ride.id ? (
                            <IonSpinner name="dots" />
                          ) : (
        ...
        {ratingRideId && (
          <IonCard style={{ margin: "12px 16px" }}>
            <IonCardContent style={{ padding: "14px 16px" }}>
              ...
              <IonItem
                lines="none"
                style={{ "--padding-start": "0", marginTop: "8px" }}>
                <IonTextarea
                  value={ratingComment}
                  onIonInput={(e) =>
                    setRatingComment(String(e.detail.value ?? ""))
                  }
              ...
              <IonItem lines="none">
                ...
                <IonToggle
                  ...
                  onIonChange={(event) =>
                    setRatingPrivateComment(event.detail.checked)
                  }
                />
              </IonItem>
              {ratingError && (
                <IonText color="danger">
                  <p style={{ fontSize: "0.82rem", margin: "4px 0" }}>
                    {ratingError}
                  </p>
                </IonText>
              )}
...
export function PassengerGuidesPage(): JSX.Element {
  ...
  const [selectedGuide, setSelectedGuide] = useState<GuidePublicData | null>(
    null,
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await touristService.listGuides(
        session.accessToken,
        filters,
      );
      setGuides(data);
      ...
      setLoadError(
        safePassengerErrorMessage(
          err instanceof Error ? err.message : "Error al cargar guías.",
        ),
      );
    } finally {
      setLoading(false);
    ...
    <IonPage>
              ...
              {(["", "es", "en", "rapa_nui"] as const).map((lang) => (
                <IonChip
                  ...
                  style={{
                    flexShrink: 0,
                    "--background":
                      filterLang === lang ? "#fff" : "rgba(255,255,255,0.2)",
                    "--color":
                      filterLang === lang ? "var(--ion-color-primary)" : "#fff",
                    fontSize: "0.76rem",
                    height: "26px",
      ...
      <IonContent>
        ...
        {!loading && guides.length === 0 && (
          <EmptyState
            icon={compassOutline}
            title="Guías locales próximamente"
            subtitle="Estamos preparando perfiles, rutas turísticas y experiencias aprobadas para Rapa Nui."
          />
        )}
                        ...
                        <div
                          ...
                          <span
                            ...
                            {rating > 0
                              ? rating.toFixed(1)
                              : "Sin calificaciones"}
                            {guide.ratingCount ? ` (${guide.ratingCount})` : ""}
                          </span>
                        </div>
                        {/* Bio */}
                        {guide.bio && (
...
function PassengerGuideDetailPage({
  guide,
  onBack,
}: {
  guide: GuidePublicData;
  onBack: () => void;
}): JSX.Element {
  const { session } = useAuth();
  ...
  const [bookingService, setBookingService] =
    useState<TouristServiceData | null>(null);
  ...
  const [pricingData, setPricingData] = useState<
    | import("../../features/tourist/tourist.service.js").ServicePricingData
    | null
  >(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    touristService
      .listGuideServices(session.accessToken, guide.id)
  ...
  useEffect(() => {
    ...
    touristService
      .getServicePricing(session.accessToken, bookingService.id)
  ...
  function computePrice(): { display: string; valid: boolean } {
    ...
    if (pricingData && pricingData.tiers.length > 0) {
      const tier = pricingData.tiers.find(
        (t) => t.minPeople <= numPeople && t.maxPeople >= numPeople,
      );
      if (tier)
        return {
          display: `$${(tier.price / 100).toLocaleString("es-CL")} CLP`,
          valid: true,
        };
      return {
        display: `Contactar operador para grupos de ${numPeople} personas`,
        valid: false,
      };
    }
    if (bookingService.price !== null) {
      return {
        display: `$${((bookingService.price * numPeople) / 100).toLocaleString("es-CL")} CLP`,
        valid: true,
      };
    }
  ...
  const LANG_LABEL_DETAIL: Record<string, string> = {
    es: "🇨🇱 Español",
    en: "🇺🇸 English",
    rapa_nui: "🗿 Rapa Nui",
  };
    ...
    <IonPage>
      ...
      <IonContent>
        ...
        <div
          style={{
            background:
              "linear-gradient(145deg, var(--ion-color-warning-shade) 0%, var(--ion-color-warning) 100%)",
          ...
          <div style={{ textAlign: "center" }}>
            <div
              style={{ color: "#fff", fontWeight: 800, fontSize: "1.15rem" }}>
              {guide.name}
            </div>
          </div>
          {/* Rating row */}
          <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
            {[1, 2, 3, 4, 5].map((n) => (
              <span
                ...
                style={{
                  fontSize: "1rem",
                  color: n <= stars ? "#fff" : "rgba(255,255,255,0.4)",
                }}>
                ★
              </span>
            ))}
        ...
        <div style={{ padding: "16px 16px 80px" }}>
          ...
          <div
            style={{ fontWeight: 700, fontSize: "1rem", marginBottom: "12px" }}>
          ...
          {!loading && services.length === 0 && (
            <EmptyState
              ...
              subtitle="Este guía no tiene servicios publicados aún"
            />
          )}
        ...
        <IonModal
          ...
          onDidDismiss={() => setBookingService(null)}>
          <IonHeader>
            <IonToolbar color="primary">
              ...
              <>
                ...
                <IonItem lines="full">
                  ...
                  <IonInput
                    ...
                    onIonInput={(e) =>
                      setBookingDate(String(e.detail.value ?? ""))
                    }
                  />
                </IonItem>
                <IonItem lines="full">
                  ...
                  <IonSelect
                    ...
                    value={bookingTime}
                    onIonChange={(e) =>
                      setBookingTime(String(e.detail.value ?? ""))
                    }
                  ...
                  <IonInput
                    ...
                    onIonInput={(e) =>
                      setNumPeople(
                        Math.max(
                          1,
                          parseInt(String(e.detail.value ?? "1"), 10),
                        ),
                      )
                    }
                  />
                ...
                <IonButton
                  ...
                  disabled={
                    ...
                    (pricingData !== null &&
                      ...
                      !pricingData.tiers.find(
                        (t) =>
                          t.minPeople <= numPeople && t.maxPeople >= numPeople,
                      ))
                  }
                  style={{ marginTop: "8px" }}>
                  {submitting ? (
        ...
        <IonToast
          ...
          color={
            toastMsg?.includes("Error") || toastMsg?.includes("Error")
              ? "danger"
              : "success"
          }
        />
...
export function PassengerServiceBookingsPage(): JSX.Element {
  ...
  async function handleCancel(bookingId: string) {
    ...
    try {
      const updated = await touristService.cancelBooking(
        session.accessToken,
        bookingId,
      );
      setBookings((prev) =>
      ...
      <IonContent className="ion-padding">
        ...
        {!loading && bookings.length === 0 && (
          <IonText color="medium">
            <p>No tienes reservas de servicios turísticos.</p>
          </IonText>
        )}
                  ...
                  <div
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div
                        ...
                        <IonBadge
                          color={BOOKING_STATUS_COLOR[b.status] ?? "medium"}
        ...
        <IonAlert
          ...
          buttons={[
            { text: "No", role: "cancel", handler: () => setConfirmId(null) },
            {
              ...
              role: "confirm",
              handler: () => {
                if (confirmId) void handleCancel(confirmId);
              },
            },
          ]}
...
export function PassengerRentalsPage(): JSX.Element {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await rentalService.listAvailableVehicles(
        session.accessToken,
        filters,
      );
              ...
              <IonCard
                ...
                {v.photos && v.photos.length > 0 ? (
                  ...
                  <div
                    style={{
                      ...
                      height: "120px",
                      background:
                        "linear-gradient(135deg, var(--ion-color-tertiary-tint) 0%, var(--ion-color-tertiary-shade) 100%)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                    }}>
                ...
                <IonCardContent style={{ padding: "14px 16px" }}>
                  ...
                  <div
                    ...
                    {v.transmission && (
                      <span>
                        ⚙️ {v.transmission === "manual" ? "Manual" : "Auto"}
                      </span>
                    )}
                    {v.fuelType && (
                      <span>
...
function PassengerRentalDetailPage({
  vehicleId,
  onBack,
}: {
  vehicleId: string;
  ...
  async function handleBook() {
    ...
    if (!startDate || !endDate) {
      setToastMsg("Selecciona fechas de inicio y fin.");
      return;
    }
    setSubmitting(true);
    try {
        ...
        <IonCard style={{ margin: "0 0 12px" }}>
          <IonCardContent style={{ padding: "12px 14px" }}>
            ...
            <div
              ...
              {vehicle.seats && (
                <span>
                  <strong>Asientos:</strong> {vehicle.seats}
                </span>
              )}
          ...
          <IonCardContent style={{ padding: "12px 14px" }}>
            ...
            <IonItem lines="full">
              ...
              <IonInput
                ...
                min={today}
                onIonInput={(e) => {
                  setStartDate(String(e.detail.value ?? ""));
                }}
              />
            </IonItem>
            ...
            <IonItem lines="full">
              ...
              <IonInput
                type="time"
                value={pickupTime}
                onIonInput={(e) => setPickupTime(String(e.detail.value ?? ""))}
              />
            </IonItem>
            ...
            <IonItem lines="full">
              ...
              <IonInput
                type="time"
                value={returnTime}
                onIonInput={(e) => setReturnTime(String(e.detail.value ?? ""))}
              />
            </IonItem>
            ...
            <IonItem lines="full">
              ...
              <IonInput
                ...
                onIonInput={(e) =>
                  setPickupLocation(String(e.detail.value ?? ""))
                }
            ...
            <IonItem lines="full">
              ...
              <IonInput
                ...
                onIonInput={(e) =>
                  setReturnLocation(String(e.detail.value ?? ""))
                }
            ...
            {days !== null && (
              <div
                ...
                {days} día{days !== 1 ? "s" : ""} × $
                {(vehicle.dailyPrice / 100).toLocaleString("es-CL")} = $
                {((vehicle.dailyPrice * days) / 100).toLocaleString("es-CL")}{" "}
                total
              </div>
            )}
...
export function PassengerRentalBookingsPage(): JSX.Element {
  ...
  async function handleCancel(bookingId: string) {
    ...
    setCancelling(bookingId);
    try {
      const updated = await rentalService.cancelRentalBooking(
        session.accessToken,
        bookingId,
      );
      setBookings((prev) =>
  ...
  return (
    <IonPage>
      ...
      <IonContent className="ion-padding">
        ...
        {!loading && bookings.length === 0 && (
          <IonText color="medium">
            <p>No tienes reservas de arriendo.</p>
          </IonText>
        )}
                    ...
                    <div
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ marginTop: "6px" }}>
                          <IonBadge
                            color={RENTAL_STATUS_COLOR[b.status] ?? "medium"}
                            style={{ fontSize: "0.7rem" }}>
...
function WalletPage(): JSX.Element {
  ...
  const [wallet, setWallet] = useState<
    import("../../features/wallet/wallet.service").WalletData | null
  >(null);
  const [transactions, setTransactions] = useState<
    import("../../features/wallet/wallet.service").TransactionData[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [loadError, setLoadError] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { walletService } =
        await import("../../features/wallet/wallet.service.js");
              ...
              <IonCard
                ...
                <IonCardContent style={{ padding: "14px" }}>
                  ...
                  <div
                    ...
                    Si un pasajero paga de más o existe una diferencia, el
                    administrador podrá revisar el caso y confirmar si ese monto
                    queda como saldo para usar en viajes futuros.
                  </div>
                </IonCardContent>
              </IonCard>
                      ...
                      <div
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div
                            ...
                            <IonBadge
                              color={TX_STATUS_COLOR[tx.status] ?? "medium"}
                        ...
                        <div
                          ...
                          {isDebit ? "−" : "+"}$
                          {(tx.amount / 100).toLocaleString("es-CL")}
                        </div>
                      </div>
                    );
                  })}
...
export function PassengerProfilePage(): JSX.Element {
  ...
  function switchToDriverMode(): void {
    ...
    window.dispatchEvent(
      new CustomEvent("rapago:role-mode-changed", {
        detail: { role: "driver" },
      }),
    );
  ...
  const loadProfile = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const data = await passengerProfileService.getMyProfile(
        session.accessToken,
      );
      setProfile(data);
  ...
  async function handleSave() {
    ...
    try {
      ...
      const updated = await passengerProfileService.upsertMyProfile(
        session.accessToken,
        payload,
      );
      setProfile(updated);
    ...
    <IonPage>
      ...
      <IonContent className="ion-padding">
        <IonRefresher
          slot="fixed"
          onIonRefresh={(e) => {
            void loadProfile().then(() => e.detail.complete());
          }}>
          <IonRefresherContent />
        </IonRefresher>
          ...
          <>
                ...
                <IonCardContent style={{ padding: "8px 14px" }}>
                  <IonText>
                    <p
                      ...
                      Tu cuenta no tiene teléfono registrado. Solicita la
                      corrección a soporte para poder solicitar viajes.
                    </p>
                  </IonText>
                </IonCardContent>
              </IonCard>
            ...
            <IonList>
              ...
              <IonItem lines="full">
                ...
                <IonInput
                  value={
                    (profile as PassengerProfileData & { rut?: string | null })
                      .rut ?? "No informado"
                  }
                  readonly
                  aria-readonly="true"
                />
                <IonNote slot="helper">
                  Nombre, correo, teléfono y RUT están bloqueados. Solo soporte
                  y administración pueden corregirlos.
                </IonNote>
              </IonItem>
              <IonButton
                expand="block"
            ...
            <IonList style={{ marginTop: "16px" }}>
              ...
              <IonItem lines="none">
                ...
                <IonSelect
                  ...
                  onIonChange={(e) =>
                    setPreferredLanguage(e.detail.value as string)
                  }>
            ...
            <IonList style={{ marginTop: "16px" }}>
              ...
              <IonItem lines="full">
                ...
                <IonInput
                  ...
                  onIonInput={(e) =>
                    setEmergencyContactName(String(e.detail.value ?? ""))
                  }
              ...
              <IonItem lines="none">
                ...
                <IonInput
                  ...
                  onIonInput={(e) =>
                    setEmergencyContactPhone(String(e.detail.value ?? ""))
                  }
            ...
            {canSwitchToDriver && (
              <IonCard
                style={{
                  ...
                  borderRadius: "18px",
                  background:
                    "linear-gradient(135deg,#111 0%,#263238 52%,#2dd36f 100%)",
                  color: "#fff",
                ...
                <IonCardContent style={{ padding: "14px 16px" }}>
                  ...
                  <div
                    ...
                    Tu cuenta fue aprobada como conductor. Puedes cambiar entre
                    pasajero y conductor sin cerrar sesión.
                  </div>
                  <IonButton
                    expand="block"
                    color="success"
