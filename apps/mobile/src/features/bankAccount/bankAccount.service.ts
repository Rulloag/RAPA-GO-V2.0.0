...
export const bankAccountService = {
  async getBankAccount(accessToken: string): Promise<BankAccountData | null> {
    ...
    if (!result.ok) throw new Error((result as { message?: string }).message ?? "Failed to load bank account.");
  ...
  async upsertBankAccount(accessToken: string, input: UpsertBankAccountInput): Promise<BankAccountData> {
    ...
    if (!result.ok) throw new Error((result as { message?: string }).message ?? "Failed to save bank account.");
