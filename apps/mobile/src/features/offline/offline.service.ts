...
export const offlineService = {
  async createOfflineBooking(accessToken: string, input: CreateOfflineBookingInput): Promise<OfflineBooking> {
    const result = await apiClient.post<BookingEnvelope>("/api/admin/offline-bookings", input, { token: accessToken });
  ...
  async listOfflineBookings(accessToken: string, status?: string): Promise<OfflineBooking[]> {
    const url = status ? `/api/admin/offline-bookings?status=${status}` : "/api/admin/offline-bookings";
  ...
  async syncOfflineBooking(accessToken: string, id: string, rideRequestId: string): Promise<OfflineBooking> {
    const result = await apiClient.patch<BookingEnvelope>(`/api/admin/offline-bookings/${id}/sync`, { rideRequestId }, { token: accessToken });
  ...
  async cancelOfflineBooking(accessToken: string, id: string): Promise<OfflineBooking> {
    const result = await apiClient.patch<BookingEnvelope>(`/api/admin/offline-bookings/${id}/cancel`, {}, { token: accessToken });
  ...
  async logConnectivity(accessToken: string, hadConnectivity: boolean, locationZone?: string): Promise<void> {
    await apiClient.post("/api/connectivity-check", { hadConnectivity, locationZone }, { token: accessToken });
  },
};
